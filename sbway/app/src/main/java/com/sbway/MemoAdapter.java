...
public class MemoAdapter extends BaseAdapter {
    ...
    public MemoAdapter( Context context, ArrayList<MemoInfo> data ) {
    ...
    public View getView(int position, View convertview, ViewGroup parent) {
        ...
        if( v == null ) {
            ...
            v = inflater.inflate( R.layout.home_memolist, null );

            // 생성된 레이아웃 객체로부터 날짜 저장요 텍스트 뷰 객체 정보를 가져와
            info.writeDate = (TextView)v.findViewById( R.id.home_memo_day );

            // 날짜객체를 이용하여 년월일을 구분하여 화면에 표시할 수 있도록
            // 가공하여 그 텍스트를 뷰에 설정한다.
            cal.setTimeInMillis( listData.get( position ).writeDate );
            info.writeDate.setText( cal.get( Calendar.YEAR ) + "." + (cal.get( Calendar.MONTH ) + 1) + "." + cal.get( Calendar.DAY_OF_MONTH ));

            // 레이아웃 객체로부터 메모 내용을 저장하기 위한 텍스트 뷰 객체 정보를 가져와
            // 메모 내용을 설정하고
            info.memo = (TextView)v.findViewById( R.id.home_memo_title );
