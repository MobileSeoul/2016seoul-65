...
public class DdayUpdate extends Activity {
    DBHandler d_handler;
    ...
    protected void d_update(){
        for(int i=0;i<d_milli_date.length;i++){
            d_handler.update(d_milli_date[i], d_memid[i], d_year[i], d_month[i], d_day[i], d_title[i], d_type[i],d_reg_date[i],d_status[i],d_aw[i]);
        }
        finish();
    }
