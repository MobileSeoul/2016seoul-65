...
public class ScheduleUpdate extends Activity {
    ...
    protected void s_update(){
        for(int i=0;i<milli_date.length;i++){
            s_handler.update(memid[i],milli_date[i],title[i],syear[i],smonth[i],sday[i],sday_week[i],stime[i],eyear[i],emonth[i],eday[i],eday_week[i],etime[i],location[i],explain[i],alarm[i],alarm_time[i],repeat[i],repeat_end[i],reg_date[i],status[i],aw[i]);
        }
        finish();
    }
